	...
	UInteger8 *ports, UInteger32 *access_delay)
{
	...
	if (as || 1 == *ports || p->transportSpecific)
		set_transparent_clock(p->clock, FALSE);
	else
		set_transparent_clock(p->clock, TRUE);
...
#ifdef MICREL_1588_PTP
	if (need_dest_port(p->clock)) {
		int port = get_master_port(p->clock);
		...
		if (port) {
			...
			if (get_hw_version(p->clock) < 2)
				msg->header.reserved1 = all_ports(p->clock);
		}
	}
#endif
...
static int port_delay_request(struct port *p)
{
	...
	if (need_dest_port(p->clock)) {
		...
		if (get_hw_version(p->clock) >= 2)
			port_set_msg_info(p, &msg->header,
				portdst(get_master_port(p->clock)), 0, 0);
	}
#endif
...
static int process_delay_req(struct port *p, struct ptp_message *m)
{
	...
	if (need_dest_port(p->clock)) {
		if (get_hw_version(p->clock) < 2)
			msg->header.reserved1 = m->header.reserved1;
		if (get_hw_version(p->clock) >= 2)
			port_set_msg_info(p, &msg->header,
				portdst(p->receive_port), 0, 0);
	}
#endif
...
static int process_pdelay_req(struct port *p, struct ptp_message *m)
{
	...
	if (!clock_two_step(p->clock)) {
		...
		if (get_hw_version(p->clock) >= 2) {
			sec = m->hwts.ts.tv_sec;
			nsec = m->hwts.ts.tv_nsec;
		}
	...
	if (need_dest_port(p->clock)) {
		if (get_hw_version(p->clock) < 2) {
			rsp->header.reserved1 = m->header.reserved1;
			fup->header.reserved1 = m->header.reserved1;
		}
		if (get_hw_version(p->clock) >= 2)
			port = p->receive_port;
	}
	if (sec || nsec || port)
		port_set_msg_info(p, &rsp->header,
			portdst(port), sec, nsec);
#endif
...
#ifdef MICREL_1588_PTP
	if (need_dest_port(p->clock) &&
			p->pdelay_resp_port == get_master_port(p->clock))
		;
	else
#endif
...
#ifdef MICREL_1588_PTP
	if (need_dest_port(p->clock) &&
			p->pdelay_resp_fup_port == get_master_port(p->clock))
		;
	else
#endif
...
#ifdef MICREL_1588_PTP
	if (p->state != PS_UNCALIBRATED && p->state != PS_SLAVE)
	pr_info("pdelay %hu   %10lld %10lld", portnum(p), p->peer_delay, pd);
#else
...
enum fsm_event port_event(struct port *p, int fd_index)
{
	...
	case PDELAY_RESP:
		...
		if (ptp_host_port &&
		    ptp_host_port != get_port_cnt(p->clock) + 1 &&
		    p->receive_port + 1 ==
		    msg->pdelay_resp.requestingPortIdentity.portNumber)
			msg->pdelay_resp.requestingPortIdentity.portNumber--;
#endif
...
static int process_delayed_sync(struct port *p, struct ptp_message *msg)
{
	...
	if (need_dest_port(p->clock)) {
		if (get_hw_version(p->clock) < 2)
			fup->header.reserved1 = msg->header.reserved1;
		if (get_hw_version(p->clock) >= 2)
			port_set_msg_info(p, &fup->header,
				portdst(msg->header.reserved1), 0, 0);
	}
	cnt = transport_send(p->trp, &p->fda, 0, fup, pdulen, &fup->hwts);
	if (cnt <= 0) {
		pr_err("port %hu: send follow up failed", portnum(p));
...
static int process_delayed_pdelay_req(struct port *p, struct ptp_message *msg)
{
	...
	memcpy(&p->peer_delay_req->hwts.ts, &msg->hwts.ts,
		sizeof(msg->hwts.ts));
	if (p->peer_delay_resp) {
		if (msg->header.reserved1 != p->pdelay_resp_port)
			return err;
...
static int process_delayed_pdelay_resp(struct port *p, struct ptp_message *msg)
{
	...
	cnt = transport_peer(p->trp, &p->fda, 0, fup, fup_len, &fup->hwts);
	if (cnt <= 0) {
		pr_err("port %hu: send pdelay_resp_fup failed", portnum(p));
		goto out;
	}
...
enum fsm_event port_tx_event(struct port *p, int fd_index)
{
	...
	cnt = transport_recv_err(p->trp, fd, msg, sizeof(msg->data),
		&msg->hwts);
	if (cnt <= 0) {
		pr_err("port %hu: recv message failed", portnum(p));
		msg_put(msg);
					  ...
					  struct ptp_message *req)
{
	...
	msg->header.logMessageInterval = 0x7f;
#ifdef MICREL_1588_PTP
	ingress->forward_port = ingress->receive_port;
	if (need_dest_port(ingress->clock)) {
		if (get_hw_version(ingress->clock) < 2)
			msg->header.reserved1 = req->header.reserved1;
	}
#endif
