...
#ifdef MICREL_1588_PTP
		c->pollfd[N_CLOCK_PFD * i + N_POLLFD].events |= POLLERR;
#endif
	}
...
#define PTP_KNOW_ABOUT_MULT_PORTS	(1 << 4)
	...
	c->dm = iface[0].dm;
	c->ieee_c37_238 = iface[0].c37_238;
	c->master_port = 0;
	do {
		int cap;
		...
		port_init_ptp(c->port[0], cap, &c->drift, &c->version,
...
int clock_poll(struct clock *c)
{
	...
	for (i = 0; i < c->nports; i++) {
		...
		for (j = err = 0; j < N_POLLFD && !err; j++) {
			k = N_CLOCK_PFD * i + j;
#ifdef MICREL_1588_PTP
			if (c->pollfd[k].revents & (POLLERR)) {
				event = port_tx_event(c->port[i], j);
#if 1
				c->pollfd[k].revents &= ~(POLLERR|POLLIN);
#endif
				   ...
				   Integer64 correction2)
{
	...
	if (c->servo_state >= SERVO_LOCKED) {
		tmv_t interval = origin - c->t1;

		if (interval < c->min_sync_interval)
...
{
	if (c->servo_state >= SERVO_LOCKED || c->dm == DM_P2P)
		return TRUE;
	else
		return FALSE;
}
