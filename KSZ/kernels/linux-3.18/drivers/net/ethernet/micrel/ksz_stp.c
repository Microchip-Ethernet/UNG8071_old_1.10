...
static int betterRootVector(struct stp_vector *first,
	struct stp_vector *second)
{
	int prio;

	prio = CMP(first->prio.root, second->prio.root);
	if (!prio)
dbg_msg("  !!! %s\n", __func__);
	return (prio < 0);
}
...
static void doFlush_(struct ksz_stp_port *p)
{
	...
	for (i = 0; i < p->br->port_cnt; i++) {
		...
		if (q == p)
			continue;
		if (q->lastPriority.port_id.num) {
			int cmp = memcmp(&q->lastPriority.bridge_id,
				&p->downPriority.bridge_id,
				sizeof(struct _bridge_id));
			if (!cmp && p->learn_jiffies) {
dbg_msg(" %ld %ld %02x%02x ", jiffies - q->alt_jiffies,
jiffies - p->learn_jiffies, q->lastPriority.port_id.prio, q->lastPriority.port_id.num);
				p->learn_jiffies = 0;
				q->lastPriority.port_id.num = 0;
				q->learn_jiffies = 0;
				q->alt_jiffies = 0;
			}
		}
...
static void stp_chk_flush(struct ksz_stp_port *p)
{
	...
	if (rcvdTc && !learning) {
		...
		for (i = 0; i < br->port_cnt; i++) {
			...
			if (q == p)
				continue;
			if (q->downPriority.port_id.num) {
				int cmp = memcmp(&q->downPriority.bridge_id,
...
dbg_msg("  %s %d=%d\n", __func__, p->port_index, flush);
	...
	if (bpduRole == ROLE_ALT_BACKUP)
		p->alt_jiffies = jiffies;
	if (bpduRole == ROLE_ROOT) {
		COPY(p->downPriority, msgPriority);
		COPY(p->lastPriority, msgPriority);
	}
#endif
}
...
#ifdef DBG_STP_PORT_BLOCK
	if (p->block_jiffies)
dbg_msg("  B:%d=%ld\n", p->port_index, jiffies - p->block_jiffies);
	p->block_jiffies = 0;
#endif
	if (learning) {
...
#define setTcPropBridge()		setTcPropTree_(p)
...
dbgPriority(root_Priority, &root_PortId);
	...
	if (!root_Priority)
		better = TRUE;
	else if (betterRootVector(&BridgePriority, &best_path))
		better = TRUE;
	if (better) {
...
static void stp_info_update_init(struct ksz_stp_port *p)
{
	...
	/* synced may be turned off here and never turned on again. */
...
static void stp_role_tr_desg_l_init(struct ksz_stp_port *p)
{
	...
	fdWhile = forwardDelay();
if (p->block_jiffies)
dbg_msg(" b: %ld %d\n", jiffies - p->block_jiffies, fdWhile);
...
static void stp_role_tr_desg_p_init(struct ksz_stp_port *p)
{
	...
	if (role != ROLE_DESIGNATED)
		dbg_stp(p, __func__, false);
#endif
if (p->block_jiffies && role != ROLE_DESIGNATED)
dbg_msg(" b: %ld %d\n", jiffies - p->block_jiffies, fdWhile);
	...
	struct ksz_stp_state *state)
{
	...
	state->change = 0;
	stp_change_state(state,
		(((rbWhile != 2 * HelloTime) && BackupPort) && canChange),
		STP_PortRoleTrans_BACKUP_PORT);
	stp_change_state(state,
		(((fdWhile != delay) || sync || reRoot || !synced) &&
		canChange),
		STP_PortRoleTrans_ALTERNATE_PORT);
	state->change = 1;
...
{
	stp_change_state(state,
		((RootPort || DesignatedPort) && forward && !operEdge),
		STP_TopologyChange_DETECTED);
	stp_change_state(state,
		((role != ROLE_ROOT) && (role != ROLE_DESIGNATED) &&
		!(learn || learning) &&
	 ...
	 * in forwarding while this port is going forwariding.
	 */
	stp_change_state(state,
		(rcvdTc || rcvdTcn || rcvdTcAck || tcProp),
		STP_TopologyChange_LEARNING);
...
#define PORT_STATE_MACHINES_BREAK	6
	...
	int duplex)
{
	...
	if (link) {
		if (!portEnabled) {
			portEnabled = TRUE;
if (p->block_jiffies) {
dbg_msg("%s %d %lu\n", __func__, p->port_index, jiffies - p->block_jiffies);
p->block_jiffies = jiffies;
}
			change = 1;
		}
...
static void stp_proc_rx(struct ksz_stp_port *p, struct bpdu *bpdu, u16 len)
{
	u16 size;
#ifdef DBG_STP_RX
	int cmp = memcmp(&p->rx_bpdu0, bpdu, sizeof(struct bpdu));
...
dbg_msg("[R] %d: ", p->port_index);
	...
	memcpy(&p->rx_bpdu0, bpdu, sizeof(struct bpdu));
#endif
...
static void proc_rx(struct work_struct *work)
{
	...
	if (stp->machine_running) {
		schedule_work(&stp->rx_proc);
		return;
	}
	last = skb_queue_empty(&stp->rxq);
	empty = last;
	while (!last) {
		skb = skb_dequeue(&stp->rxq);
		last = skb_queue_empty(&stp->rxq);
		if (skb) {
			int port;
			struct bpdu *bpdu;
			...
			if (bpdu)
				stp_proc_rx(&br->ports[port], bpdu, len);
...
static void stp_stop(struct ksz_stp_info *stp)
{
	int i;
	struct ksz_stp_port *p;
	struct ksz_sw *sw = stp->sw_dev;
	...
	char *buf)
{
	...
	case PROC_GET_STP_BR_INFO:
		...
		len += sprintf(buf + len,
			" time since topology change\t%4u\n",
			0);
		len += sprintf(buf + len,
			" topology change count\t\t%4u\n",
			0);
		len += sprintf(buf + len,
			" topology change\t\t%4s\n",
			BOOL_STR(0));
	...
	ssize_t len, char *buf)
{
	...
	case PROC_GET_STP_INFO:
		if (p->off)
			break;
		state_str = get_port_state_str(p->states[7]);
		len += sprintf(buf + len,
			" enabled\t\t%4s\n",
