...
static int mrp_set_bandwidth(struct mrp_port_info *info)
{
	...
	bandwidth = calculate_max_bandwidth(info->speed,
		info->bandwidth[0].deltaBandwidth +
		info->bandwidth[1].deltaBandwidth);
	...
	bandwidth_left = 0;
	for (n = 0, tc = SR_CLASS_A; tc >= SR_CLASS_B; tc--, n++) {
		bandwidth = calculate_max_bandwidth(info->speed,
			info->bandwidth[n].deltaBandwidth);
...
static void mrp_init(struct mrp_info *mrp)
{
	...
	for (port = 0; port < sw->mib_port_cnt; port++) {
		...
		info->bandwidth[0].traffic_class = SR_CLASS_A;
		info->bandwidth[0].deltaBandwidth = 50;
		info->bandwidth[0].adminIdleSlope = 0;

		info->bandwidth[1].traffic_class = SR_CLASS_B;
		info->bandwidth[1].deltaBandwidth = 25;
		info->bandwidth[1].adminIdleSlope = 0;
